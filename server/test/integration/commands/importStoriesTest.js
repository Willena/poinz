...
import {prepOneUserInOneRoom, textToCsvDataUrl, textToJsonDataUrl} from '../../testUtils.js';
...
describe('csv file', () => {
  ...
  test('should throw on invalid command payload format', async () => {
    ...
    return expect(
      ...
      /Format validation failed \(must be a valid text\/csv or application\/json data url\) in \/payload\/data/
    );
  });
...
describe('json file', () => {
  test('Should produce correct events for all stories in data', async () => {
    ...
    expect(producedEvents).toMatchEvents(
      ...
      // finally select first (non-trashed) story
      'storySelected'
    );
  });
});
